...
export function wrapWithScopeContext() {
  return (tree, file) => {
    ...
    const ids = imports.reduce<string[]>(
      (identifiers: string[], importSpecifier: ImportSpecifier) => {
        const newIds: string[] = [];
    ...
    tree.children.unshift({
      type: 'import',
      value: `import { MDXScopeProvider } from '@waweb/mdx.ui.mdx-scope-context';`,
    });
...
function createCompiler(options: Partial<MDXCompileOptions>) {
  ...
  const compilerOpts = Object.assign(options, {
    remarkPlugins: options.remarkPlugins
      ? mustPlugins.concat(options.remarkPlugins)
      : mustPlugins,
  ...
  return function transformer(tree, file) {
    visit(tree, 'yaml', (node: any) => {
      try {
        ...
        throw new Error(
          `failed extracting metadata/front-matter using Yaml lib, due to an error: ${err.message}`
        );
      }
    });
  };
...
function extractImports() {
  return function transformer(tree, file) {
    visit(tree, 'import', (node: any) => {
      ...
      const imports: ImportSpecifier[] = Object.keys(es6Import).flatMap(
        (dep) => {
